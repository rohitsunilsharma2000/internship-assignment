...
| 3️⃣ | 🔑 **JWT Token Tampering**                        | User claims a higher role in token                      | Reject with `403 Forbidden`, must validate against DB/secure source. | ❌ validation error is not working for null or empty role              |
| 4️⃣ | 🔐 **User Has No Role (Unassigned)**              | Logged-in user has no role                              | Deny creation with message: "User role not assigned."                | ❌              |
| 5️⃣ | 🏛️ **HR\_MANAGER Role Not Configured in System** | Role logic missing in backend enum/map                  | Return 500 or fail gracefully with admin alert(🏛️ Unrecognized HR_MANAGER Role in Backend).                      | ❌              |
